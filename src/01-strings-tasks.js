...
function getRectangleString(width, height) {
  const rectangle = new Array(height).fill(new Array(width).fill(0));
  return rectangle.map((line, i1) => line.map((col, i2) => {
    if (i1 === 0) {
      if (i2 === 0) return '┌';
      if (i2 === line.length - 1) return '┐\n';
      return '─';
    }
    if (i1 === rectangle.length - 1) {
      if (i2 === 0) return '└';
      if (i2 === line.length - 1) return '┘\n';
      return '─';
    }
...
function isString(value) {
  const alphabet = 'abcdefghijklmnopqrstuvwxyz';
  try {
    return value.split('').every((x) => alphabet.includes(x));
  } catch (e) {
    return false;
  }
}
